...
def step_impl(context, vote_type):
    ...
    ActionChains(context.browser).click(button).pause(1).perform() # Pause is needed bc of slow scroll
...
def step_impl(context, amount, value):
    ...
    value = value == "True"
    assert ReviewVote.objects.all(lambda review: review.value == value), "There were votes not matching the given value {}".format(value)
